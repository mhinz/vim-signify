...
" Function: s:sign_define {{{1
function! s:sign_define(name, linehl)
  function! SignNameToText(s)
    let s = a:s[7:]
    let s = substitute(s, '\(\u\)', '_\l\1', 'g')
    let s = 'sign' . s
    return s
  endfunction
  function! SignNameToTexthl(s)
    return a:s[0:12]
  endfunction
  let text = get(s:, SignNameToText(a:name))
  let texthl = SignNameToTexthl(a:name)
  execute 'sign define '.a:name.' text='.text.' texthl='.texthl.' linehl='.a:linehl
endfunction
...
call s:sign_define('SignifyAdd',              'none')
call s:sign_define('SignifyChange',           'none')
call s:sign_define('SignifyChangeDelete',     'none')
call s:sign_define('SignifyDelete',           'none')
call s:sign_define('SignifyDeleteFirstLine',  'none')
...
function! s:line_highlighting_enable() abort
  ...
  call s:sign_define('SignifyAdd',              add)
  call s:sign_define('SignifyChange',           change)
  call s:sign_define('SignifyChangeDelete',     change)
  call s:sign_define('SignifyDelete',           delete)
  call s:sign_define('SignifyDeleteFirstLine',  delete)
